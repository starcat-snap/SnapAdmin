...
import type { SnapAdminDiscountCampaignService } from 'src/app/service/discount-campaign.service';
import type {
...
export default class SnapAdminExtensionService {
    ...
    constructor(
        ...
        private readonly discountCampaignService: SnapAdminDiscountCampaignService,
    ...
    private async getLinkToTheme(extension: Extension) {
        ...
        const themeRepository = SnapAdmin.Service('repositoryFactory').create('theme');
    ...
    private getAppFromStore(extensionName: string) {
        return SnapAdmin.State.get('shopwareApps').apps.find((innerApp) => {
