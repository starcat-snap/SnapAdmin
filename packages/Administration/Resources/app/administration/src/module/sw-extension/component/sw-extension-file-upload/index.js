...
export default {
    ...
    methods: {
        ...
        getUserConfig() {
            return this.userConfigRepository.search(this.userConfigCriteria, SnapAdmin.Context.api).then(response => {
                if (response.length) {
                    this.pluginUploadUserConfig = response.first();
                } else {
                    this.pluginUploadUserConfig = this.userConfigRepository.create(SnapAdmin.Context.api);
        ...
        saveConfig(value) {
            ...
            this.userConfigRepository.save(this.pluginUploadUserConfig, SnapAdmin.Context.api).then(() => {
                this.getUserConfig();
            });
        },
    },
