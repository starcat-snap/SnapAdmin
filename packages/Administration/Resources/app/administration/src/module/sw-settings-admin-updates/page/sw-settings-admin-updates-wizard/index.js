import template from './sw-settings-admin-updates-wizard.html.twig';
import './sw-settings-admin-updates-wizard.scss';
 ...
 * @private
 */
Component.register('sw-settings-admin-updates-wizard', {
    template,
    ...
    computed: {
        ...
        updateButtonTooltip() {
            ...
            return {
                message: this.$tc('sw-settings-admin-updates.infos.requirementsNotMet'),
                position: 'bottom',
            };
        },
        ...
        optionDeactivateIncompatibleTranslation() {
            const deactivateIncompatTrans = this.$tc('sw-settings-admin-updates.plugins.actions.deactivateIncompatible');
            const isRecommended = this.displayIncompatiblePluginsWarning && !this.displayUnknownPluginsWarning ?
                this.$tc('sw-settings-admin-updates.plugins.actions.recommended') : '';

            return `${deactivateIncompatTrans} ${isRecommended}`;
        },

        optionDeactivateAllTranslation() {
            const deactiveAllTrans = this.$tc('sw-settings-admin-updates.plugins.actions.deactivateAll');
            const isRecommended = this.displayIncompatiblePluginsWarning && this.displayUnknownPluginsWarning ?
                this.$tc('sw-settings-admin-updates.plugins.actions.recommended') : '';
    ...
    methods: {
        ...
        startUpdateProcess() {
            ...
            this.createNotificationSuccess({
                message: this.$tc('sw-settings-admin-updates.notifications.updateStarted'),
            });
        ...
        stopUpdateProcess() {
            ...
            this.createNotificationInfo({
                message: this.$tc('sw-settings-admin-updates.notifications.updateStopped'),
            });
        },
        ...
        downloadRecovery(offset) {
            this.updateService.downloadRecovery(offset).then(() => {
                ...
                this.createNotificationError({
                    message: this.$tc('sw-settings-admin-updates.notifications.downloadFailed'),
                });
            });
        },
        ...
        deactivatePlugins(offset) {
            ...
            this.updateService.deactivatePlugins(offset, this.chosenPluginBehaviour).then(response => {
                ...
                if (response.offset === response.total) {
                    this.redirectToPage(`${SnapAdmin.Context.api.basePath}/admin-installer.phar.php`);
                } else {
                ...
                if (context.code === 'FRAMEWORK__PLUGIN_HAS_DEPENDANTS') {
                    ...
                    this.createNotificationError({
                        message: this.$tc('sw-settings-admin-updates.notifications.deactivationFailed'),
                    });
                }
            });
        },
