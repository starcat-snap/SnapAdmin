...
SnapAdmin.Service().register('flowBuilderService', () => {
    return new FlowBuilderService();
});

SnapAdmin.Component.register('sw-flow-sequence-action', swFlowSequenceAction);
...
async function createWrapper(propsData = {}, appFlowResponseData = [], flag = '') {
    ...
    const wrapper = shallowMount(await SnapAdmin.Component.build('sw-flow-sequence-action'), {
        localVue,
        stubs: {
            'sw-icon': {
            ...
            'sw-grouped-single-select': await SnapAdmin.Component.build('sw-grouped-single-select'),
        ...
        provide: {
            ...
            flowBuilderService: SnapAdmin.Service('flowBuilderService'),
        },
    });
    wrapper.vm.$refs = {
        contextButton: [
...
describe('src/module/sw-flow/component/sw-flow-sequence-action', () => {
        ...
        SnapAdmin.State.registerModule('swFlowState', {
            ...
            state: {
                ...
                triggerEvent: {
                    ...
                    aware: [
                        'SnapAdmin\\Core\\Framework\\Event\\CustomerAware',
                        'SnapAdmin\\Core\\Framework\\Event\\OrderAware',
                        'SnapAdmin\\Core\\Framework\\Event\\MailAware',
                    ],
                },
                triggerActions: [
                    { name: 'action.add.order.tag', requirements: ['SnapAdmin\\Core\\Framework\\Event\\OrderAware'], extensions: [] },
                    { name: 'action.add.customer.tag', requirements: ['SnapAdmin\\Core\\Framework\\Event\\CustomerAware'], extensions: [] },
                    { name: 'action.remove.customer.tag', requirements: ['SnapAdmin\\Core\\Framework\\Event\\CustomerAware'], extensions: [] },
                    { name: 'action.remove.order.tag', requirements: ['SnapAdmin\\Core\\Framework\\Event\\OrderAware'], extensions: [] },
                    { name: 'action.mail.send', requirements: ['SnapAdmin\\Core\\Framework\\Event\\MailAware'], extensions: [] },
                    { name: 'action.set.order.state', requirements: ['SnapAdmin\\Core\\Framework\\Event\\OrderAware'], extensions: [] },
                    { name: 'telegram.send.message', requirements: ['SnapAdmin\\Core\\Framework\\Event\\CustomerAware'], extensions: [] },
    ...
    it('should set error for single select if action name is empty', async () => {
        SnapAdmin.State.commit('swFlowState/setInvalidSequences', ['2']);
    ...
    it('should correct label in set order state description', async () => {
        SnapAdmin.State.commit('swFlowState/setStateMachineState', [
            {
    ...
    it('should has actions from app flow actions in actions list', async () => {
        ...
        SnapAdmin.State.commit('swFlowState/setAppActions', [{
            label: 'Telegram send message',
            name: 'telegram.send.message',
            swIcon: 'default-communication-speech-bubbles',
            requirements: ['customerAware', 'orderAware'],
