...
const mockBusinessEvents = [
    {
        ...
        mailAware: true,
        aware: ['SnapAdmin\\Core\\Framework\\Event\\SalesChannelAware'],
    },
    {
...
async function createWrapper(propsData) {
    ...
    return shallowMount(await SnapAdmin.Component.build('sw-flow-trigger'), {
        localVue,
        mocks: {
            $tc(translationKey) {
        ...
        stubs: {
            ...
            'sw-tree': await SnapAdmin.Component.build('sw-tree'),
            'sw-tree-item': await SnapAdmin.Component.build('sw-tree-item'),
            'sw-loader': true,
            'sw-icon': {
                template: '<div></div>',
            },
            'sw-vnode-renderer': await SnapAdmin.Component.build('sw-vnode-renderer'),
...
describe('src/module/sw-flow/component/sw-flow-trigger', () => {
    beforeAll(() => {
        ...
        SnapAdmin.Service().register('businessEventService', () => {
            return {
                getBusinessEvents: () => Promise.resolve(mockBusinessEvents),
            };
        });

        SnapAdmin.State.registerModule('swFlowState', flowState);
    });
    ...
    it('should display event tree with event get from flow state', async () => {
        SnapAdmin.State.commit('swFlowState/setTriggerEvents', mockBusinessEvents);
        ...
        eventTree = wrapper.find('.sw-tree');
        expect(eventTree.exists()).toBeTruthy();
        SnapAdmin.State.commit('swFlowState/setTriggerEvents', []);
    });
    ...
    it('should show confirmation modal when clicking tree item', async () => {
        ...
        const isSequenceEmpty = SnapAdmin.State.getters['swFlowState/isSequenceEmpty'];
    ...
    it('should get event tree data correctly with custom event', async () => {
        ...
        mockBusinessEvents.push({
            ...
            mailAware: true,
            aware: ['SnapAdmin\\Core\\Framework\\Event\\CustomEventAware'],
        });
