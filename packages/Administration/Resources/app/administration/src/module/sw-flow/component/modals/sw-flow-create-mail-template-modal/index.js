...
export default {
    ...
    methods: {
        createdComponent() {
            if (!SnapAdmin.State.getters['context/isSystemDefaultLanguage']) {
                SnapAdmin.State.commit('context/resetLanguageToDefault');
            }

            this.mailTemplate = this.mailTemplateRepository.create(SnapAdmin.Context.api, utils.createId());
        },
        ...
        getMailTemplate() {
            return this.mailTemplateRepository.get(this.mailTemplate.id, SnapAdmin.Context.api, this.mailTemplateCriteria)
                .then((data) => {
