...
SnapAdmin.Component.register('sw-flow-grant-download-access-modal', swFlowGrantDownloadAccessModal);
...
async function createWrapper(config = null) {
    ...
    return shallowMount(await SnapAdmin.Component.build('sw-flow-grant-download-access-modal'), {
        localVue,
...
describe('module/sw-flow/component/sw-flow-grant-download-access-modal', () => {
    ...
    it('should return error if value is null', async () => {
        ...
        expect(wrapper.vm.fieldError(wrapper.vm.value)).toBeInstanceOf(SnapAdminError);
    });
