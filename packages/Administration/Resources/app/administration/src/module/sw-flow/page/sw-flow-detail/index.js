...
const { Component, Mixin, Context, State, Utils, Service } = SnapAdmin;
const { Criteria, EntityCollection } = SnapAdmin.Data;
const { cloneDeep } = SnapAdmin.Utils.object;
...
export default {
    ...
    methods: {
        ...
        getDetailFlow() {
            this.isLoading = true;
            SnapAdmin.State.dispatch('swFlowState/fetchTriggerActions');
        ...
        getAppFlowAction() {
            return this.appFlowActionRepository.search(this.appFlowActionCriteria, SnapAdmin.Context.api)
                .then((response) => {
        ...
        getDataForActionDescription() {
            ...
            if (hasDocumentAction) {
                ...
                promises.push(this.documentTypeRepository.search(this.documentTypeCriteria).then((data) => {
                    SnapAdmin.State.commit('swFlowState/setDocumentTypes', data);
                }));
            }
            ...
            if (hasMailSendAction) {
                ...
                promises.push(this.mailTemplateRepository.search(this.mailTemplateIdsCriteria).then((data) => {
                    SnapAdmin.State.commit('swFlowState/setMailTemplates', data);
                }));
            }
            ...
            if (hasChangeCustomerGroup) {
                ...
                promises.push(this.customerGroupRepository.search(this.customerGroupCriteria).then((data) => {
                    SnapAdmin.State.commit('swFlowState/setCustomerGroups', data);
                }));
            }
            ...
            if (hasSetCustomFieldAction) {
                promises.push(this.customFieldSetRepository.search(this.customFieldSetCriteria).then((data) => {
                    SnapAdmin.State.commit('swFlowState/setCustomFieldSets', data);
                }));

                promises.push(this.customFieldRepository.search(this.customFieldCriteria).then((data) => {
                    SnapAdmin.State.commit('swFlowState/setCustomFields', data);
                }));
            }
