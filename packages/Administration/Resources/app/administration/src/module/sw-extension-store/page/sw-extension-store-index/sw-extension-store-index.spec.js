...
SnapAdmin.Application.addServiceProvider('extensionErrorService', () => {
...
async function createWrapper() {
    ...
    return shallowMount(await SnapAdmin.Component.build('sw-extension-store-index'), {
        localVue,
        propsData: {},
        mocks: {
            $tc: v => v,
        ...
        stubs: {
            'sw-meteor-page': await SnapAdmin.Component.build('sw-meteor-page'),
            'sw-search-bar': {
...
describe('SwagExtensionStore/module/sw-extension-store/page/sw-extension-store-index', () => {
    ...
    beforeEach(async () => {
        SnapAdmin.State.get('snapAdminExtensions').search.filter = {};
    ...
    it('should filter to only app extensions', async () => {
        ...
        const filter = SnapAdmin.State.get('snapAdminExtensions').search.filter;
    ...
    it('should show listing errors on listing errors event', async () => {
        ...
        expect(wrapper.find('sw-extension-store-error-card-stub').attributes().title)
            .toBe('SnapAdmin version is unknown');
        ...
        expect(wrapper.find('sw-extension-store-error-card-stub').text())
            .toBe('The given SnapAdmin version is unknown, please contact our customer service');
    });
});
