...
export default {
    ...
    computed: {
        recommendedVariants() {
            return this.snapAdminExtensionService.orderVariantsByRecommendation(this.extension.variants);
        },
        ...
        formattedPrice() {
            ...
            return Utils.format.currency(
                this.snapAdminExtensionService.getPriceFromVariant(this.selectedVariant),
                'EUR',
            );
        },
        ...
        purchaseButtonLabel() {
            switch (this.selectedVariant.type) {
                case this.snapAdminExtensionService.EXTENSION_VARIANT_TYPES.FREE:
                    return this.$tc('sw-extension-store.component.sw-extension-buy-modal.purchaseButtonsLabels.free');

                case this.snapAdminExtensionService.EXTENSION_VARIANT_TYPES.RENT:
                    return this.$tc('sw-extension-store.component.sw-extension-buy-modal.purchaseButtonsLabels.rent');

                case this.snapAdminExtensionService.EXTENSION_VARIANT_TYPES.BUY:
                default:
        ...
        vatIncludedClasses() {
            return {
                'is--hidden': this.selectedVariant.type === this.snapAdminExtensionService.EXTENSION_VARIANT_TYPES.FREE,
            };
        },

        renewalDateClasses() {
            return {
                'is--hidden': this.selectedVariant.type !== this.snapAdminExtensionService.EXTENSION_VARIANT_TYPES.RENT,
            };
        },
        ...
        userCanBuyFromStore() {
            ...
            return SnapAdmin.State.get('snapAdminExtensions').userInfo !== null;
        },
    ...
    methods: {
        ...
        variantRecommendation(variant) {
            return this.snapAdminExtensionService.mapVariantToRecommendation(variant);
        },
        ...
        async getPaymentMeans() {
            this.extensionStoreLicensesService.getPaymentMeans().then((response) => {
                ...
                if (!Array.isArray(errorMessages)) {
                    SnapAdmin.Utils.debug.warn('Payment loading error', error);
                    return;
                }
