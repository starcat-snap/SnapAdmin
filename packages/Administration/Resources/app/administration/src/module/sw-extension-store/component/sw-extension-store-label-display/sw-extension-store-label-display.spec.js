...
async function createWrapper(labels) {
    return shallowMount(await SnapAdmin.Component.build('sw-extension-store-label-display'), {
        propsData: {
            labels
        },
        stubs: {
            'sw-extension-label': await SnapAdmin.Component.build('sw-extension-label')
        }
    });
}
