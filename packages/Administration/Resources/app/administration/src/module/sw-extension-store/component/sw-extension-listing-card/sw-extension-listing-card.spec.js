...
async function createWrapper(extension) {
    return shallowMount(await SnapAdmin.Component.build('sw-extension-listing-card'), {
        propsData: {
            extension
        },
        mocks: {
        ...
        provide: {
            ...
            snapAdminExtensionService: new SnapAdminExtensionService(
                undefined,
                undefined,
                new SnapAdminDiscountCampaignService()
            )
        }
    });
}
...
describe('sw-extension-listing-card', () => {
    ...
    beforeEach(() => {
        SnapAdmin.State.commit('snapAdminExtensions/setExtension', {
            name: 'Test',
            installedAt: null
        });
    });
