...
async function createWrapper(extensionCustomProps = {}, canBeOpened = true) {
    ...
    return shallowMount(await SnapAdmin.Component.build('sw-extension-store-detail'), {
        localVue,
        propsData: {
            id: 'a1b2c3'
        },
        ...
        stubs: {
            ...
            'sw-button-process': await SnapAdmin.Component.build('sw-button-process'),
            'sw-alert': true,
...
describe('SwagExtensionStore/module/sw-extension-store/page/sw-extension-store-detail', () => {
    ...
    beforeEach(() => {
        SnapAdmin.State.get('session').languageId = 'b2c3d4';
        SnapAdmin.State.get('snapAdminExtensions').myExtensions = {
            loading: false,
            data: [{
                active: true,
                name: 'SwagB2BPlatform',
    ...
    afterEach(() => {
        SnapAdmin.State.get('session').languageId = '';
        SnapAdmin.State.get('snapAdminExtensions').myExtensions = { data: [], loading: false };
    });
