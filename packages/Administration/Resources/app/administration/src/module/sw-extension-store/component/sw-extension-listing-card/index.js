    ...
    computed: {
        ...
        calculatedPrice() {
            ...
            return this.$tc(
                ...
                this.snapAdminExtensionService.mapVariantToRecommendation(this.recommendedVariant),
                {
                    price: Utils.format.currency(
                        this.snapAdminExtensionService.getPriceFromVariant(this.recommendedVariant),
                        'EUR'
                    )
                }
            );
        ...
        isInstalled() {
            return !!SnapAdmin.State.get('snapAdminExtensions').myExtensions.data.some((installedExtension) => {
                return installedExtension.installedAt && installedExtension.name === this.extension.name;
            });
        },

        isLicensed() {
            const extension = SnapAdmin.State.get('snapAdminExtensions').myExtensions.data
