...
export default {
    ...
    computed: {
        ...
        extensionMetaData() {
            ...
            return SnapAdmin.State.get('snapAdminExtensions').myExtensions.data.find((extension) => {
        ...
        recommendedVariant() {
            return this.snapAdminExtensionService.orderVariantsByRecommendation(this.extension.variants)[0];
        },

        dateFilter() {
            return SnapAdmin.Filter.getByName('date');
        },

        hasActiveDiscount() {
            return this.snapAdminExtensionService.isVariantDiscounted(this.recommendedVariant);
        },
        ...
        discountedPrice() {
            return this.snapAdminExtensionService.getPriceFromVariant(this.recommendedVariant);
        },
    ...
    async created() {
        ...
        this.canBeOpened = !!this.snapAdminExtensionService.getOpenLink(this.extension);
    },
    ...
    methods: {
        async fetchExtension() {
            ...
            try {
                ...
                this.extension = await this.extensionStoreDataService.getDetail(
                    this.id,
                    { ...SnapAdmin.Context.api, languageId: this.languageId }
                );
            } catch (error) {
                ...
                if (errorData.code === 'FRAMEWORK__STORE_ERROR' && errorData.title === 'Extension unknown') {
                    ...
                    const docLink = this.$tc('sw-extension.errors.messageToTheSnapAdminDocumentation', 0, errorData.meta);
        ...
        async onClickAddExtension() {
            ...
            if (!SnapAdmin.State.get('snapAdminExtensions').userInfo) {
        ...
        async openExtension() {
            const openLink = await this.snapAdminExtensionService.getOpenLink(this.extension);
