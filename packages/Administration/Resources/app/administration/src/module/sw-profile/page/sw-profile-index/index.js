...
const { Component, Mixin, State } = SnapAdmin;
...
export default {
    ...
    methods: {
        createdComponent() {
            ...
            this.timezoneOptions = SnapAdmin.Service('timezoneService').getTimezoneOptions();
        ...
        loadLanguages() {
            ...
            return this.languageRepository.search(languageCriteria).then((result) => {
                ...
                result.forEach((lang) => {
                    ...
                    if (lang.locale.code === SnapAdmin.Context.app.fallbackLocale) {
        ...
        saveUser(context) {
            if (!this.acl.can('user:editor')) {
                ...
                this.userService.updateUser(changes.changeset[0].changes).then(async () => {
                    ...
                    SnapAdmin.Service('localeHelper').setLocaleWithId(this.user.localeId);
                }).catch((error) => {
                    State.dispatch('error/addApiError', {
                        expression: `user.${this.user?.id}.password`,
                        error: new SnapAdmin.Classes.SnapAdminError(error.response.data.errors[0]),
                    });
                    this.createNotificationError({
