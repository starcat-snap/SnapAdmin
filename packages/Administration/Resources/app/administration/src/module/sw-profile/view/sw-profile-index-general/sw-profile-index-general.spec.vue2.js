...
SnapAdmin.Component.register('sw-profile-index-general', swProfileIndexGeneral);

async function createWrapper(privileges = []) {
    const localVue = createLocalVue();

    return shallowMount(await SnapAdmin.Component.build('sw-profile-index-general'), {
        localVue,
        stubs: {
            'sw-card': true,
            'sw-container': true,
...
describe('src/module/sw-profile/view/sw-profile-index-general', () => {
    ...
    it('should be able to change new password', async () => {
        ...
        await wrapper.setData({
            computedNewPassword: 'SnapAdmin',
        });

        expect(spyNewPasswordChangeEmit).toHaveBeenCalledWith('new-password-change', 'SnapAdmin');
    });
    ...
    it('should be able to change new password confirm', async () => {
        ...
        await wrapper.setData({
            computedNewPasswordConfirm: 'SnapAdmin',
        });

        expect(spyNewPasswordConfirmChangeEmit).toHaveBeenCalledWith('new-password-confirm-change', 'SnapAdmin');
    });
