...
const { Component, Mixin, Context } = SnapAdmin;
...
export default {
    ...
    methods: {
        loadConditionData() {
            const context = { ...Context.api, languageId: SnapAdmin.State.get('session').languageId };
            const criteria = new Criteria();
        ...
        onDuplicate() {
            return this.onSave().then(() => {
                ...
                return this.ruleRepository.clone(this.rule.id, behaviour, SnapAdmin.Context.api).then((duplicatedData) => {
                    this.$router.push(
                        {
