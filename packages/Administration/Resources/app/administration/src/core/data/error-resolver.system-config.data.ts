...
export default class ErrorResolverSystemConfig {
    ...
    public cleanWriteErrors() {
        void SnapAdmin.State.dispatch('error/resetApiErrors');
    }

    private reduceErrorsByWriteIndex(errors: ApiError[]) {
        const writeErrors: {
            systemError: SnapAdminError[],
            apiError: {
                [key: string]: SnapAdminError
            },
        } = {
            systemError: [],
            apiError: {},
        ...
        errors.forEach((current) => {
            if (!current.source || !current.source.pointer) {
                const systemError = new SnapAdminError({
                    code: current.code,
                    meta: current.meta,
            ...
            segments.reduce((pointer: {[key: string]: Partial<SnapAdminError> }, segment, index) => {
                // skip translations
                ...
                if (index === lastIndex) {
                    pointer[segment] = new SnapAdminError(current);
                } else {
                    pointer[segment] = {};
                }
    ...
    private addSystemErrors(errors: SnapAdminError[]) {
        errors.forEach((error) => {
            void SnapAdmin.State.dispatch('error/addSystemError', error);
        });
    }

    private handleErrors(errors: {[key: string]: SnapAdminError}) {
        Object.keys(errors).forEach((key: string) => {
            void SnapAdmin.State.dispatch('error/addApiError', {
