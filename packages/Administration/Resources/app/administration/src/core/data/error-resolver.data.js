...
export default class ErrorResolver {
    constructor() {
        this.EntityDefinition = SnapAdmin.EntityDefinition;
        this.SnapAdminError = SnapAdmin.Classes.SnapAdminError;
        this.merge = SnapAdmin.Utils.object.merge;
    }

    resetApiErrors() {
        return SnapAdmin.State.dispatch('error/resetApiErrors');
    }
    ...
    handleDeleteError(errors) {
        errors.forEach(({ error, entityName, id }) => {
            const shopwareError = new this.SnapAdminError(error);
            SnapAdmin.State.dispatch('error/addSystemError', { error: shopwareError });

            SnapAdmin.State.dispatch('error/addApiError', {
    ...
    reduceErrorsByWriteIndex(errors) {
        ...
        errors.forEach((current) => {
            if (!current.source || !current.source.pointer) {
                writeErrors.system.push(new this.SnapAdminError(current));
                return;
            }
            ...
            segments.reduce((pointer, segment, index) => {
                ...
                if (index === lastIndex) {
                    pointer[segment] = new this.SnapAdminError(current);
                } else {
                    pointer[segment] = {};
                }
    ...
    addSystemErrors(systemErrors) {
        systemErrors.forEach((error) => {
            SnapAdmin.State.dispatch('error/addSystemError', error);
        });
    }
    ...
    resolveError(fieldName, error, definition, entity, changeset) {
        ...
        if (!(error instanceof this.SnapAdminError)) {
            error = new this.SnapAdminError(error);
        }
    ...
    buildAssociationChangeset(entity, changeset, error, associationName) {
        if (!changeset || !SnapAdmin.Utils.object.hasOwnProperty(changeset, associationName)) {
            SnapAdmin.State.dispatch('error/addApiError', {
    ...
    resolveJsonFieldError(basePath, error) {
        Object.keys(error).forEach((fieldName) => {
            ...
            if (error[fieldName] instanceof this.SnapAdminError) {
                SnapAdmin.State.dispatch('error/addApiError', {
                    expression: path,
