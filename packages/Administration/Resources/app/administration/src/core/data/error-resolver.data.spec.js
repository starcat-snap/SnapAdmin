...
describe('src/core/data/error-resolver.data', () => {
    ...
    describe('resetApiErrors', () => {
        it('should dispatches "error/resetApiErrors" action', () => {
            ...
            expect(SnapAdmin.State.dispatch).toHaveBeenCalledWith('error/resetApiErrors');
        });
    });
    ...
    describe('handleWriteErrors', () => {
        ...
        it('should handles write errors and adds system errors', () => {
            ...
            expect(SnapAdmin.State.dispatch).toHaveBeenCalledTimes(3);
            expect(SnapAdmin.State.dispatch).toHaveBeenNthCalledWith(1, 'error/addApiError', {
                expression: expect.anything(),
                error: expect.any(SnapAdmin.Classes.SnapAdminError),
            });
            expect(SnapAdmin.State.dispatch).toHaveBeenNthCalledWith(2, 'error/addApiError', {
                expression: expect.anything(),
                error: expect.any(SnapAdmin.Classes.SnapAdminError),
            });
            expect(SnapAdmin.State.dispatch).toHaveBeenNthCalledWith(3, 'error/addSystemError', expect.any(SnapAdmin.Classes.SnapAdminError));
        });

        it('should convert to SnapAdminError', () => {
            const errors = [
                { source: { pointer: '/0/firstName' }, code: 'CODE1' },
            ];
    ...
    describe('handleDeleteError', () => {
        it('should handle delete errors and add system errors and api errors', () => {
            ...
            expect(SnapAdmin.State.dispatch).toHaveBeenCalledWith('error/addSystemError', {
                error: expect.any(SnapAdmin.Classes.SnapAdminError),
            });
            expect(SnapAdmin.State.dispatch).toHaveBeenCalledWith('error/addApiError', {
                expression: 'Entity1.1', error: expect.any(SnapAdmin.Classes.SnapAdminError),
            });
            expect(SnapAdmin.State.dispatch).toHaveBeenCalledWith('error/addApiError', {
                expression: 'Entity2.2', error: expect.any(SnapAdmin.Classes.SnapAdminError),
            });
        });
    });
});
