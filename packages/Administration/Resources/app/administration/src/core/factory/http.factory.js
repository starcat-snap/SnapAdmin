...
function createClient() {
    const client = Axios.create({
        baseURL: SnapAdmin.Context.api.apiPath,
    });
        ...
        try {
            ...
            if (errors) {
                errors.forEach(singleError => {
                    SnapAdmin.State.dispatch('notification/createNotification', {
                        variant: 'error',
...
/**
 * Determines the different status codes and creates a matching error via SnapAdmin.State
 * @param {Number} status
 * @param {Array} errors
 * @param {Object} error
 * @param {Object} data
...
function handleErrorStates({ status, errors, error = null, data }) {
    ...
    const viewRoot = SnapAdmin.Application.view.root;
    ...
    if (status === 403) {
        ...
        missingPrivilegeErrors.forEach(missingPrivilegeError => {
            ...
            SnapAdmin.State.dispatch('notification/createNotification', {
                ...
                autoClose: false,
                growl: true,
                title: SnapAdmin.Snippet.tc('global.error-codes.FRAMEWORK__MISSING_PRIVILEGE_ERROR'),
                message: `${SnapAdmin.Snippet.tc('sw-privileges.error.description')} <br> ${missingPrivilegesMessage}`,
            });
        });
    }
    ...
    if (status === 403
        ...
        SnapAdmin.State.dispatch('notification/createNotification', {
            ...
            autoClose: false,
            growl: true,
            title: SnapAdmin.Snippet.tc('sw-extension.errors.storeSessionExpired.title'),
            message: SnapAdmin.Snippet.tc('sw-extension.errors.storeSessionExpired.message'),
            actions: [{
                label: SnapAdmin.Snippet.tc('sw-extension.errors.storeSessionExpired.actionLabel'),
                method: () => {
    ...
    if (status === 409) {
        if (errors[0].code === 'FRAMEWORK__DELETE_RESTRICTED') {
            ...
            blockingEntities = parameters.usages.reduce((message, usageObject) => {
                ...
                const timesSnippet = SnapAdmin.Snippet.tc('global.default.xTimesIn', times);
                const blockingEntitiesSnippet = SnapAdmin.Snippet.tc(`global.entities.${usageObject.entityName}`, times[1]);
                return `${message}<br>${timesSnippet} <b>${blockingEntitiesSnippet}</b>`;
            }, '');

            SnapAdmin.State.dispatch('notification/createNotification', {
                variant: 'error',
                title: SnapAdmin.Snippet.tc('global.default.error'),
                message: `${SnapAdmin.Snippet.tc(
                    'global.notification.messageDeleteFailed',
                    3,
                    { entityName: SnapAdmin.Snippet.tc(`global.entities.${entityName}`) },
                )
                }${blockingEntities}`,
            });
        }
        ...
        if (status === 401) {
            ...
            return new Promise((resolve, reject) => {
                tokenHandler.subscribe(
                    ...
                    (err) => {
                        if (!SnapAdmin.Application.getApplicationRoot()) {
                            reject(err);
...
function tracingInterceptor(client) {
    ...
    if (process.env.NODE_ENV !== 'test') {
        client.interceptors.request.use((config) => {
            const currentRoute = SnapAdmin?.Application?.view?.router?.history?.current?.name;
