...
class ApplicationBootstrapper {
     ...
     *
     * @example
     * SnapAdmin.Application.addFactory('module', (container) => {
     ...
     *
     * @example
     * SnapAdmin.Application.addFactoryMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * SnapAdmin.Application.addFactoryMiddleware('module', (service, next) => {
     ...
     *
     * @example
     * SnapAdmin.Application.addFactoryDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * SnapAdmin.Application.addFactoryDecorator('module', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
     ...
     *
     * @example
     * SnapAdmin.Application.addInitializer('httpClient', (container) => {
     ...
     *
     * @example
     * SnapAdmin.Application.addServiceProvider('productService', (container) => {
    ...
    registerApiContext(context: ContextState['api']): ApplicationBootstrapper {
        SnapAdmin.Context.api = SnapAdmin.Classes._private.ApiContextFactory(context);
    ...
    registerAppContext(context: ContextState['app']): ApplicationBootstrapper {
        SnapAdmin.Context.app = SnapAdmin.Classes._private.AppContextFactory(context);
     ...
     *
     * @example
     * SnapAdmin.Application.addServiceProviderMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * SnapAdmin.Application.addServiceProviderMiddleware('productService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    initializeFeatureFlags(): ApplicationBootstrapper {
        const features = SnapAdmin.Context.app.features;
        ...
        SnapAdmin.Feature.init(features);
     ...
     *
     * @example
     * SnapAdmin.Application.addServiceProviderDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * SnapAdmin.Application.addServiceProviderDecorator('productService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    bootFullApplication(): Promise<void | ApplicationBootstrapper> {
        ...
        return this.initializeInitializers(initContainer)
            ...
            .then(() => Promise.all(SnapAdmin.Plugin.getBootPromises()))
            .then(() => {
    ...
    createApplicationRoot(): Promise<ApplicationBootstrapper> {
        ...
        if (SnapAdmin.Context.app.environment === 'testing') {
        ...
        const firstRunWizard = SnapAdmin.Context.app.firstRunWizard;
        // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
    ...
    private async loadPlugins():Promise<(unknown[] | null)[]> {
        ...
        if (isDevelopmentMode === 'development') {
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            plugins = SnapAdmin.Context.app.config.bundles as bundlesPluginResponse;
        }
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        const bundles = SnapAdmin.Context.app.config.bundles as bundlesPluginResponse;
        Object.entries(bundles).forEach(([bundleName, bundle]) => {
            ...
            if (isDevelopmentMode === 'development') {
                ...
                Object.entries(plugins).forEach(([pluginName, entryFiles]) => {
                    const stringUtils = SnapAdmin.Utils.string;
    ...
    private injectIframe({
        ...
        const bundles = SnapAdmin.Context.app.config.bundles;
        ...
        SnapAdmin.State.commit('extensions/addExtension', extension);
    }
}
