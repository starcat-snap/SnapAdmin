...
describe('core/application.js', () => {
    const originalInjectJs = SnapAdmin.Application.injectJs;

    beforeEach(() => {
        SnapAdmin.Application.injectJs = originalInjectJs;
        SnapAdmin.Context.app.config.bundles = {};
    });

    it('should be error tolerant if loading a plugin\'s files fails', async () => {
        const warningSpy = jest.spyOn(console, 'warn').mockImplementation();

        SnapAdmin.Application.injectJs = async () => {
            throw new Error('Inject js fails');
        };

        const result = await SnapAdmin.Application.injectPlugin({
            js: ['some.js'],
        });
    ...
    it('should call swagCommercial before any other plugins', async () => {
        ...
        SnapAdmin.Application.injectPlugin = async (plugin) => {
            callOrder.js.push(plugin.js);
            callOrder.css.push(plugin.css);
        };

        // load all plugins
        await SnapAdmin.Application.loadPlugins();
