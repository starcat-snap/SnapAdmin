...
describe('src/core/service/usage-data-consent-listener.service.ts', () => {
    ...
    beforeEach(() => {
        ...
        loginService = createLoginService({}, SnapAdmin.Context.api);
        serviceContainer = {
            usageDataService: {
                getConsent: () => {
        ...
        SnapAdmin.State.commit('usageData/updateConsent', {
    ...
    it('should update consent on login', async () => {
        ...
        expect(SnapAdmin.State.get('usageData').isConsentGiven).toBe(true);
        expect(SnapAdmin.State.get('usageData').isBannerHidden).toBe(false);
    });
    ...
    it('should reset the consent if the request fails', async () => {
        ...
        expect(SnapAdmin.State.get('usageData').isConsentGiven).toBe(false);
        expect(SnapAdmin.State.get('usageData').isBannerHidden).toBe(true);
    });
