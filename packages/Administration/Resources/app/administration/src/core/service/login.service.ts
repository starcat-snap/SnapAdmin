...
export default function createLoginService(
    ...
    function verifyUserToken(password: string): Promise<string> {
        ...
        return verifyUserByUsername(SnapAdmin.State.get('session').currentUser.username, password)
            .then(({ access }) => {
                if (SnapAdmin.Utils.types.isString(access)) {
                    return access;
                }
    ...
    function shouldConsiderUserActivity(): boolean {
        ...
        const devEnv = SnapAdmin.Context.app.environment === 'development';
    ...
    function forwardLogout(isInactivityLogout: boolean, shouldRedirect: boolean): void {
        ...
        const router = SnapAdmin.Application.view.router as null | Router;
        if (router) {
            const id = SnapAdmin.Utils.createId();
            ...
            if (isInactivityLogout && shouldRedirect) {
                ...
                void html2canvas(document.querySelector('#app'), { scale: 0.1 }).then(canvas => {
                    ...
                    window.sessionStorage.setItem('lastKnownUser', SnapAdmin.State.get('session').currentUser.username);
