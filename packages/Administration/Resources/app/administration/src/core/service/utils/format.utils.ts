...
export function currency(
    ...
    const opts = {
        style: 'currency',
        currency: sign || SnapAdmin.Context.app.systemCurrencyISOCode as string,
        ...decimalOpts,
        ...additionalOptions,
    };

    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access,@typescript-eslint/no-unsafe-argument
    return val.toLocaleString((additionalOptions.language ?? SnapAdmin.State.get('session').currentLocale) ?? 'en-US', opts);
}
...
export function date(val: string, options: DateFilterOptions = {}): string {
    ...
    const lastKnownLang = SnapAdmin.Application.getContainer('factory').locale.getLastKnownLocale();
