...
async function createWrapper({
    ...
    return shallowMount({
        ...
        `,
        mixins: [
            SnapAdmin.Mixin.getByName('listing'),
        ],
        data() {
            return {
                ...defaultData,
...
describe('src/app/mixin/listing.mixin.ts', () => {
    ...
    beforeEach(async () => {
        ...
        if (originalWarn) {
            SnapAdmin.Utils.debug.warn = originalWarn;
        } else {
            originalWarn = SnapAdmin.Utils.debug.warn;
        }
    });
    ...
    it('should throw a console warning when no getList method ist defined', async () => {
        ...
        SnapAdmin.Utils.debug.warn = jest.fn(() => {});
        ...
        expect(SnapAdmin.Utils.debug.warn).toHaveBeenCalledWith(
            'Listing Mixin',
