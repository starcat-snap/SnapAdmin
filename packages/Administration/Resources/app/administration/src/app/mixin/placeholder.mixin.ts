 ...
 * @private
 */
export default SnapAdmin.Mixin.register('placeholder', defineComponent({
    methods: {
        placeholder<EntityName extends keyof EntitySchema.Entities>(
            ...
            if (SnapAdmin.Utils.types.isString(entity[field]) && entity[field].length > 0) {
                return entity[field];
            }

            // TODO: Refactor with NEXT-3304
            // Return the field from parent translation if set
            const parentLanguageId = SnapAdmin.Context.api.language ? SnapAdmin.Context.api.language.parentId : null;
