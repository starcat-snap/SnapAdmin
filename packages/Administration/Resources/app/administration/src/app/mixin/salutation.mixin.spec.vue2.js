...
async function createWrapper() {
    return shallowMount({
        ...
        `,
        mixins: [
            SnapAdmin.Mixin.getByName('salutation'),
        ],
        data() {
            return {
                name: 'sw-mock-field',
...
describe('src/app/mixin/salutation.mixin.ts', () => {
    ...
    beforeEach(async () => {
        if (originalFilterGetByName) {
            Object.defineProperty(SnapAdmin.Filter, 'getByName', {
                value: jest.fn(() => {
                    return jest.fn(() => 'Salutation filter result');
                }),
            });
        } else {
            originalFilterGetByName = SnapAdmin.Filter.getByName;
        }
    ...
    it('should compute correct salutationFilter value', () => {
        ...
        expect(SnapAdmin.Filter.getByName).toHaveBeenCalledWith('salutation');
    });
