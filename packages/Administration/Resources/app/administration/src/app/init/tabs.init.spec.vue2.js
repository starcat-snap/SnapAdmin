...
describe('src/app/init/tabs.init', () => {
    ...
    beforeAll(() => {
        ...
        // Mock component
        SnapAdmin.Application.view.getComponent = () => ({});
        ...
        SnapAdmin.Application.view.router = routerMock;
    ...
    beforeEach(async () => {
        // Reset tab store
        Object.keys(SnapAdmin.State.get('tabs').tabItems).forEach(key => {
            Vue.set(SnapAdmin.State.get('tabs').tabItems, key, []);
        });
    });
    ...
    it('should initialize tab extension API correctly', async () => {
        ...
        expect(SnapAdmin.State.get('tabs').tabItems).toHaveProperty('foo-position-id');
        expect(SnapAdmin.State.get('tabs').tabItems['foo-position-id']).toEqual([{
            label: 'My tab item',
            componentSectionId: 'foo-component-section-id',
        }]);
    });
    ...
    it('should create correct route entry for tab item when route gets opened (added via beforeEach middleware)', async () => {
        ...
        // initialize view
        await SnapAdmin.Application._resolveViewInitialized();
