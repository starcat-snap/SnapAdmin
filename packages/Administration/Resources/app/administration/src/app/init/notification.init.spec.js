...
import { notification } from '@snap-admin/admin-extension-sdk';
...
describe('src/app/init/notification.init.ts', () => {
    ...
    beforeEach(() => {
        SnapAdmin.State.get('notification').growlNotifications = {};
    });
    ...
    it('should handle notificationDispatch requests', async () => {
        ...
        const growlNotificationKey = Object.keys(SnapAdmin.State.get('notification').growlNotifications)[0];
        expect(SnapAdmin.State.get('notification').growlNotifications).toEqual({
