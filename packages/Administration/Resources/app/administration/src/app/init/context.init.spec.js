...
import { getCurrency, getEnvironment, getLocale, getSnapAdminVersion, getModuleInformation, getAppInformation, getUserInformation } from '@snap-admin/admin-extension-sdk/es/context';
...
describe('src/app/init/context.init.ts', () => {
    ...
    beforeEach(() => {
        if (SnapAdmin.State.get('extensions')) {
            SnapAdmin.State.unregisterModule('extensions');
        }

        SnapAdmin.State.registerModule('extensions', extensionsStore);
    });
    ...
    it('should handle shopware version', async () => {
        await getSnapAdminVersion().then((version) => {
