...
export default function initializeTabs(): void {
    SnapAdmin.ExtensionAPI.handle('uiTabsAddTabItem', (componentConfig) => {
        SnapAdmin.State.commit('tabs/addTabItem', componentConfig);

        // if current route does not exist check if they exists after adding the route
        const router = SnapAdmin.Application.view?.router as Router;
    ...
    /* istanbul ignore next */
    void SnapAdmin.Application.viewInitialized.then(() => {
...
function createRouteForTabItem(to: RouteLocationNormalized, router: Router, next: () => void): boolean {
    ...
    Object.values(SnapAdmin.State.get('tabs').tabItems).find((tabItemConfigs) => {
    ...
    if (parentRoute && parentRoute.children) {
        ...
        if (!routeAlreadyExists) {
            const newRoute: RouteRecordRaw = {
                path: dynamicPath,
                // @ts-expect-error
                component: SnapAdmin.Application.view.getComponent('sw-extension-component-section'),
                name: newRouteName,
                meta: {
