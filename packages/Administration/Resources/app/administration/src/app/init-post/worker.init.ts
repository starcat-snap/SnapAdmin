...
import type { ApiContext } from '@snap-admin/admin-extension-sdk/es/data/_internals/EntityCollection';
...
export default function initializeWorker() {
    ...
    const context = SnapAdmin.Context.app;
    const workerNotificationFactory = SnapAdmin.Application.getContainer('factory').workerNotification;
    const configService = SnapAdmin.Service('configService');
    ...
    function getConfig() {
        return configService.getConfig().then((response) => {
            Object.entries(response as { [key: string]: unknown}).forEach(([key, value]) => {
                SnapAdmin.State.commit('context/addAppConfigValue', { key, value });
            });
            ...
            if (context.config.adminWorker?.enableAdminWorker && !enabled) {
                enableAdminWorker(loginService, SnapAdmin.Context.api, context.config.adminWorker);
            }
        });
    }
...
function registerThumbnailMiddleware(factory: typeof WorkerNotificationFactory) {
    ...
    factory.register('DalIndexingMessage', {
        name: 'SnapAdmin\\Core\\Framework\\DataAbstractionLayer\\Indexing\\MessageQueue\\IndexerMessage',
    ...
    factory.register('EsIndexingMessage', {
        name: 'SnapAdmin\\Elasticsearch\\Framework\\Indexing\\IndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('generateThumbnailsMessage', {
        name: 'SnapAdmin\\Core\\Content\\Media\\Message\\GenerateThumbnailsMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('PromotionIndexingMessage', {
        name: 'SnapAdmin\\Core\\Checkout\\Promotion\\DataAbstractionLayer\\PromotionIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('ProductStreamIndexingMessage', {
        name: 'SnapAdmin\\Core\\Content\\ProductStream\\DataAbstractionLayer\\ProductStreamIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('CategoryIndexingMessage', {
        name: 'SnapAdmin\\Core\\Content\\Category\\DataAbstractionLayer\\CategoryIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('MediaIndexingMessage', {
        name: 'SnapAdmin\\Core\\Content\\Media\\DataAbstractionLayer\\MediaIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('ProductIndexingMessage', {
        name: 'SnapAdmin\\Core\\Content\\Product\\DataAbstractionLayer\\ProductIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('ElasticSearchIndexingMessage', {
        name: 'SnapAdmin\\Elasticsearch\\Framework\\Indexing\\ElasticsearchIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification('esIndexing', ids, next, entry, $root, notification, {
                title: 'global.default.success',
                message: 'global.notification-center.worker-listener.esIndexing.message',
    ...
    factory.register('ImportExportMessage', {
        name: 'SnapAdmin\\Core\\Content\\ImportExport\\Message\\ImportExportMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('FlowIndexingMessage', {
        name: 'SnapAdmin\\Core\\Content\\Flow\\Indexing\\FlowIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('NewsletterRecipientIndexingMessage', {
        name: 'SnapAdmin\\Core\\Content\\Newsletter\\DataAbstractionLayer\\NewsletterRecipientIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
