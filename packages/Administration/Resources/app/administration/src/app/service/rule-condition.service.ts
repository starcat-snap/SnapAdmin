import type EntityCollection from '@snap-admin/admin-extension-sdk/es/data/_internals/EntityCollection';
...
export default class RuleConditionService {
    ...
    getRestrictedRules(entityName: string) {
        ...
        const ruleRepository = SnapAdmin.Service('repositoryFactory').create('rule');
    ...
    getTranslatedConditionViolationList(violations: Array<{ label: string }>, connectionSnippetPath: string) {
        const app = SnapAdmin.Application.getApplicationRoot();
        if (!app) {
            return '';
        }
