...
export default class FilterService {
    ...
    getStoredFilters(storeKey) {
        ...
        return this._userConfigRepository.search(criteria, SnapAdmin.Context.api).then(response => {
            if (response.length) {
                this._filterEntity = response.first();
            } else {
                const currentUser = SnapAdmin.State.get('session').currentUser;

                this._filterEntity = this._userConfigRepository.create(SnapAdmin.Context.api);
    ...
    saveFilters(storeKey, filters) {
        ...
        this._pushFiltersToUrl();
        this._userConfigRepository.save(this._filterEntity, SnapAdmin.Context.api).then(() => {
    ...
    async _pushFiltersToUrl() {
        ...
        const router = SnapAdmin.Application.view.router;
        ...
        if (!SnapAdmin.Utils.types.isEmpty(routeParams)) {
