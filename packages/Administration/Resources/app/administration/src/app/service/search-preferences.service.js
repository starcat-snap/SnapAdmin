...
export default function SearchPreferencesService({ userConfigRepository: _userConfigRepository }) {
    ...
    */
    function getDefaultSearchPreferences() {
        ...
        SnapAdmin.Module.getModuleRegistry().forEach(({ manifest }) => {
            if (
                manifest.entity &&
                SnapAdmin.Service('acl').can(`${manifest.entity}.editor`) &&
    ...
    */
    function getUserSearchPreferences() {
        return new Promise((resolve) => {
            SnapAdmin.Service('userConfigService').search([KEY_USER_SEARCH_PREFERENCE]).then((response) => {
    ...
    */
    function processSearchPreferencesFields(tempSearchPreferencesFields) {
        ...
        tempSearchPreferencesFields.forEach((field) => {
            field.group.forEach((group) => {
                const searchPreferencesField = SnapAdmin.Utils.object.set({}, group.fieldName, {
                    _searchable: field._searchable,
                    _score: field._score,
                });
                searchPreferencesFields = SnapAdmin.Utils.object.deepMergeObject(
    ...
    function _getUserConfigCriteria() {
        const criteria = new SnapAdmin.Data.Criteria();

        criteria.addFilter(SnapAdmin.Data.Criteria.equals('key', KEY_USER_SEARCH_PREFERENCE));
        criteria.addFilter(SnapAdmin.Data.Criteria.equals('userId', _getCurrentUser()?.id));
