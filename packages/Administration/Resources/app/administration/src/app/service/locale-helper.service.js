...
export default class LocaleHelperService {
    ...
    constructor({ SnapAdmin, localeRepository, snippetService, localeFactory }) {
        this._SnapAdmin = SnapAdmin;
    ...
    async setLocaleWithId(localeId) {
        const { code } = await this._localeRepository.get(localeId, this._SnapAdmin.Context.api);
    ...
    async setLocaleWithCode(localeCode) {
        ...
        await this._SnapAdmin.State.dispatch('setAdminLocale', localeCode);
    }
}
