...
describe('src/app/service/entity-validation.service.js', () => {
    beforeAll(() => {
        Object.entries(entitySchemaMock).forEach(([entityName, definitionData]) => {
            SnapAdmin.EntityDefinition.add(entityName, new EntityDefinition(definitionData));
        });
    });
    ...
    it('should validate a product and report callback errors', () => {
        ...
        expect(customValidator.mock.calls[0][2]).toBe(SnapAdmin.EntityDefinition.get(testEntity.getEntityName())); // entity definition
