...
describe('app/service/locale-helper.service.js', () => {
    ...
    beforeEach(async () => {
        localeHelperService = new LocaleHelperService({
            SnapAdmin: { Context: { api: {} }, State: { dispatch: () => Promise.resolve() } },
    ...
    it('setLocaleWithCode should dispatch the admin locale', async () => {
        localeHelperService._SnapAdmin.State.dispatch = jest.fn();

        await localeHelperService.setLocaleWithCode('testCode');

        expect(localeHelperService._SnapAdmin.State.dispatch).toHaveBeenCalledWith('setAdminLocale', 'testCode');
    });
});
