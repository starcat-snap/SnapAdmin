...
Component.register('sw-app-actions', {
    ...
    computed: {
        ...
        params() {
            return SnapAdmin.State.get('shopwareApps').selectedIds;
        },
        ...
        extensionSdkButtons() {
            return SnapAdmin.State.get('actionButtons').buttons.filter((button) => {
    ...
    methods: {
        ...
        getUserConfig() {
            this.userConfigRepository.search(this.userConfigCriteria, SnapAdmin.Context.api).then(response => {
                if (response.length) {
                    this.iframeUserConfig = response.first();
                } else {
                    this.iframeUserConfig = this.userConfigRepository.create(SnapAdmin.Context.api);
        ...
        saveConfig(value) {
            ...
            this.userConfigRepository.save(this.iframeUserConfig, SnapAdmin.Context.api).then(() => {
                this.getUserConfig();
            });
        },
    },
