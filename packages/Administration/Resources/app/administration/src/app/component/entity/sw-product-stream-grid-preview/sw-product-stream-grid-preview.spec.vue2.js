...
const createWrapper = async () => {
    ...
    return shallowMount(await SnapAdmin.Component.build('sw-product-stream-grid-preview'), {
        localVue,
        stubs: {
            'sw-empty-state': await SnapAdmin.Component.build('sw-empty-state'),
            'sw-simple-search-field': await SnapAdmin.Component.build('sw-simple-search-field'),
...
describe('components/entity/sw-product-stream-grid-preview.spec', () => {
    ...
    it('should send request with term when updating searchTerm', async () => {
        ...
        wrapper.destroy();
        SnapAdmin.Utils.debounce = jest.fn((fn) => fn);
        wrapper = await createWrapper();
        ...
        SnapAdmin.Utils.debounce.mockReset();
    });
