...
const menuService = createMenuService(SnapAdmin.Module);
SnapAdmin.Service().register('menuService', () => menuService);
...
async function createWrapper(options = {}) {
    ...
    const adminMenuComponent = await SnapAdmin.Component.build('sw-admin-menu');

    return shallowMount(adminMenuComponent, {
        localVue,
        router: new VueRouter({ routes: SnapAdmin.Module.getModuleRoutes(), route: { meta: { $module: { name: '' } } } }),
        stubs: {
            'sw-icon': true,
            'sw-version': true,
            'sw-admin-menu-item': await SnapAdmin.Component.build('sw-admin-menu-item'),
            'sw-loader': true,
            'sw-avatar': true,
...
describe('src/app/component/structure/sw-admin-menu', () => {
    ...
    beforeAll(() => {
        ...
        SnapAdmin.State.registerModule('settingsItems', {
            namespaced: true,
            state: {
                settingsGroups: {
                    shop: [],
    ...
    beforeEach(async () => {
        jest.spyOn(SnapAdmin.Utils.debug, 'error').mockImplementation(() => true);

        SnapAdmin.State.commit('setCurrentUser', null);
        SnapAdmin.State.get('settingsItems').settingsGroups.shop = [];
        SnapAdmin.State.get('settingsItems').settingsGroups.system = [];

        SnapAdmin.Module.getModuleRegistry().clear();
    ...
    it('should not render 4.level or higher menu item and throw error', async () => {
        ...
        expect(SnapAdmin.Utils.debug.error.mock.calls[0][0]).toBeInstanceOf(Error);
        expect(SnapAdmin.Utils.debug.error.mock.calls[0][0].toString()).toBe(
        ...
        expect(SnapAdmin.Utils.debug.error.mock.calls[1][0]).toBeInstanceOf(Error);
        expect(SnapAdmin.Utils.debug.error.mock.calls[1][0].toString()).toBe(
