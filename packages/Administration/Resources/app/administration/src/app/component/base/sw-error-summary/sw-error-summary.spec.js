...
async function createWrapper(errors = {}, options = {}) {
    if (typeof SnapAdmin.State.get('error') !== 'undefined') {
        SnapAdmin.State.unregisterModule('error');
    }

    SnapAdmin.State.registerModule('error', {
        namespaced: true,
    ...
    SnapAdmin.State.getters['error/getAllApiErrors'] = () => [errors];
