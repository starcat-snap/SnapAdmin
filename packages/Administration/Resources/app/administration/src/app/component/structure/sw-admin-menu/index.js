...
const { dom, types } = SnapAdmin.Utils;
...
Component.register('sw-admin-menu', {
    ...
    computed: {
        currentUser() {
            return SnapAdmin.State.get('session').currentUser;
        },

        isExpanded() {
            return SnapAdmin.State.get('adminMenu').isExpanded;
        },
        ...
        currentLocale() {
            return SnapAdmin.State.get('session').currentLocale;
        },

        currentExpandedMenuEntries() {
            return SnapAdmin.State.get('adminMenu').expandedEntries;
        },

        adminModuleNavigation() {
            const adminModuleNavigationEntries = SnapAdmin.State.get('adminMenu').adminModuleNavigation;
            ...
            return adminModuleNavigationEntries.filter((entry) => {
                ...
                if (levelThreeParent) {
                    SnapAdmin.Utils.debug.error(new Error(
...
The admin menu only supports up to three levels of nesting.`,
        ...
        appModuleNavigation() {
            return SnapAdmin.State.getters['adminMenu/appModuleNavigation'];
        },
        ...
        mainMenuEntries() {
            const tree = new SnapAdmin.Helper.FlatTreeHelper((first, second) => first.position - second.position);
    ...
    methods: {
        ...
        initNavigation() {
            SnapAdmin.State.commit('adminMenu/setAdminModuleNavigation', this.menuService.getNavigationFromAdminModules());
        ...
        collapseAdminMenu() {
            SnapAdmin.State.commit('adminMenu/collapseSidebar');
        },

        expandAdminMenu() {
            SnapAdmin.State.commit('adminMenu/expandSidebar');
        },
        ...
        getUser() {
            ...
            this.userService.getUser().then((response) => {
                ...
                SnapAdmin.State.commit('setCurrentUser', userData);
        ...
        onLogoutUser() {
            ...
            SnapAdmin.State.commit('adminMenu/clearExpandedMenuEntries');
            SnapAdmin.State.commit('removeCurrentUser');
            SnapAdmin.State.commit('notification/setNotifications', {});
            SnapAdmin.State.commit('notification/clearGrowlNotificationsForCurrentUser');
        ...
        onMenuItemClick(entry, eventTarget) {
            ...
            if (isEntryExpanded) {
                SnapAdmin.State.commit('adminMenu/collapseMenuEntry', entry);

                firstChild.classList.remove('router-link-active');
                firstChild.classList.remove('is--entry-expanded');
            } else {
                SnapAdmin.State.commit('adminMenu/expandMenuEntry', entry);
