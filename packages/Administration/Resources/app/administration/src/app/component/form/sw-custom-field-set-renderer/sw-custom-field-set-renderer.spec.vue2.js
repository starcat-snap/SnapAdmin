...
SnapAdmin.Component.register('sw-media-field', SwMediaField);
SnapAdmin.Component.register('sw-media-media-item', SwMediaMediaItem);
SnapAdmin.Component.register('sw-media-base-item', SwMediaBaseItem);
SnapAdmin.Component.register('sw-media-preview-v2', SwMediaPreviewV2);

function createEntityCollection(entities = []) {
    return new SnapAdmin.Data.EntityCollection('collection', 'collection', {}, null, entities);
}
...
async function createWrapper(props) {
    ...
    localVue.filter('mediaName', SnapAdmin.Filter.getByName('mediaName'));

    return shallowMount(await SnapAdmin.Component.build('sw-custom-field-set-renderer'), {
        localVue,
        propsData: props,
        stubs: {
            'sw-button': await SnapAdmin.Component.build('sw-button'),
            'sw-label': await SnapAdmin.Component.build('sw-label'),
            'sw-tabs': await SnapAdmin.Component.build('sw-tabs'),
            'sw-tabs-item': await SnapAdmin.Component.build('sw-tabs-item'),
            'sw-inherit-wrapper': await SnapAdmin.Component.build('sw-inherit-wrapper'),
            'sw-inheritance-switch': await SnapAdmin.Component.build('sw-inheritance-switch'),
            'sw-form-field-renderer': await SnapAdmin.Component.build('sw-form-field-renderer'),
            ...
            'sw-number-field': await SnapAdmin.Component.build('sw-number-field'),
            ...
            'sw-field-error': await SnapAdmin.Component.build('sw-field-error'),
            'sw-icon': {
                template: '<div class="sw-icon" @click="$emit(\'click\')"></div>',
            },
            'sw-single-select': await SnapAdmin.Component.build('sw-single-select'),
            'sw-multi-select': await SnapAdmin.Component.build('sw-multi-select'),
            ...
            'sw-select-selection-list': await SnapAdmin.Component.build('sw-select-selection-list'),
            'sw-popover': await SnapAdmin.Component.build('sw-popover'),
            'sw-highlight-text': await SnapAdmin.Component.build('sw-highlight-text'),
            'sw-media-field': await SnapAdmin.Component.build('sw-media-field'),
            'sw-media-media-item': await SnapAdmin.Component.build('sw-media-media-item'),
            'sw-media-base-item': await SnapAdmin.Component.build('sw-media-base-item'),
            'sw-media-preview-v2': await SnapAdmin.Component.build('sw-media-preview-v2'),
            'sw-colorpicker': await SnapAdmin.Component.build('sw-text-field'),
            'sw-upload-listener': true,
            'sw-simple-search-field': true,
            'sw-loader': true,
            'sw-datepicker': await SnapAdmin.Component.build('sw-text-field'),
            'sw-text-editor': {
                props: ['value'],
                template: '<input type="text" :value="value" @change="$emit(\'change\', $event.target.value)"></input>',
            },
            'sw-skeleton': await SnapAdmin.Component.build('sw-skeleton'),
            'sw-skeleton-bar': await SnapAdmin.Component.build('sw-skeleton-bar'),
        },
        provide: {
            repositoryFactory: {
                create: (entity) => ({
