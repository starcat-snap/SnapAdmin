...
Component.register('sw-notifications', {
    ...
    computed: {
        notifications() {
            return Object.values(SnapAdmin.State.getters['notification/getGrowlNotificationsObject']);
        },
    ...
    methods: {
        onClose(notification) {
            SnapAdmin.State.commit('notification/removeGrowlNotification', notification);
        },

        handleAction(action, notification) {
            // Allow external links for example to the shopware account or store
            if (SnapAdmin.Utils.string.isUrl(action.route)) {
