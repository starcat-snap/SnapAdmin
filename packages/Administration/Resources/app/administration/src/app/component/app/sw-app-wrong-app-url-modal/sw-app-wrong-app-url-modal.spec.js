...
describe('sw-app-wrong-app-url-modal', () => {
    ...
    beforeAll(() => {
        ...
        SnapAdmin.State.registerModule('context', {
            namespaced: true,
            state: {
                app: {
                    config: {
        ...
        SnapAdmin.State.unregisterModule('notification');

        SnapAdmin.State.registerModule('notification', {
            namespaced: true,
            mutations: {
    ...
    it('should show modal', async () => {
        SnapAdmin.State.get('context').app.config.settings.appUrlReachable = false;
        SnapAdmin.State.get('context').app.config.settings.appsRequireAppUrl = true;
    ...
    it('should not show modal if APP_URL is reachable', async () => {
        SnapAdmin.State.get('context').app.config.settings.appUrlReachable = true;
        SnapAdmin.State.get('context').app.config.settings.appsRequireAppUrl = true;
        localStorage.removeItem(STORAGE_KEY_WAS_WRONG_APP_MODAL_SHOWN);
    ...
    it('should not show modal if no apps are require app url, but it should show notification', async () => {
        SnapAdmin.State.get('context').app.config.settings.appUrlReachable = false;
        SnapAdmin.State.get('context').app.config.settings.appsRequireAppUrl = false;
        localStorage.removeItem(STORAGE_KEY_WAS_WRONG_APP_MODAL_SHOWN);
