...
SnapAdmin.Component.register('sw-extension-icon', SwExtensionIcon);
...
describe('sw-app-actions', () => {
    ...
    async function createWrapper(router, resultData = actionResultData) {
        ...
        return mount(await SnapAdmin.Component.build('sw-app-actions'), {
            localVue,
            stubs,
    ...
    beforeAll(async () => {
        stubs = {
            'sw-app-action-button': await SnapAdmin.Component.build('sw-app-action-button'),
            'sw-icon': await SnapAdmin.Component.build('sw-icon'),
            'sw-context-button': await SnapAdmin.Component.build('sw-context-button'),
            'sw-context-menu': await SnapAdmin.Component.build('sw-context-menu'),
            'sw-context-menu-item': await SnapAdmin.Component.build('sw-context-menu-item'),
            ...
            'sw-extension-icon': await SnapAdmin.Component.build('sw-extension-icon'),
        };
    });

    beforeEach(async () => {
        SnapAdmin.Application.view.deleteReactive = () => {};
        SnapAdmin.State.commit('shopwareApps/setSelectedIds', [SnapAdmin.Utils.createId()]);
    ...
    it('calls appActionButtonService.runAction if triggered by context menu button', async () => {
        ...
        expect(runActionsMock.mock.calls[0]).toEqual([
            ...
            { ids: SnapAdmin.State.get('shopwareApps').selectedIds },
        ]);
    ...
    it('calls appActionButtonService.runAction with correct response', async () => {
        ...
        const actionButtonId = SnapAdmin.Utils.createId();
