 ...
 */
SnapAdmin.Component.register('sw-iframe-renderer', {
    template,
    ...
    created() {
        this.heightHandler = SnapAdmin.ExtensionAPI.handle('locationUpdateHeight', ({ height, locationId }) => {
            if (locationId === this.locationId) {
                this.locationHeight = Number(height) ?? null;
            }
        });

        this.urlHandler = SnapAdmin.ExtensionAPI.handle('locationUpdateUrl', async ({
            hash,
            pathname,
            searchParams,
            locationId,
    ...
    computed: {
        ...
        componentName(): string|undefined {
            return SnapAdmin.State.get('sdkLocation').locations[this.locationId];
        },

        extension(): Extension | undefined {
            const extensions = SnapAdmin.State.get('extensions');
