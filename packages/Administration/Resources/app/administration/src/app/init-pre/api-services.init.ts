...
const apiServices = SnapAdmin._private.ApiServices();
...
export default function initializeApiServices() {
    ...
    apiServices.forEach((ApiService) => {
        ...
        const initContainer = SnapAdmin.Application.getContainer('init');

        const apiServiceFactory = factoryContainer.apiService;
        // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-assignment
        const service = new ApiService(initContainer.httpClient, SnapAdmin.Service('loginService'));
        ...
        SnapAdmin.Application.addServiceProvider(serviceName, () => {
