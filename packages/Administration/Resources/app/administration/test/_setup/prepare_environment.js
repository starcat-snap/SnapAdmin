...
// Add all directives
const directiveRegistry = SnapAdmin.Directive.getDirectiveRegistry();
...
// Add services
SnapAdmin.Service().register('acl', () => aclService);
SnapAdmin.Service().register('feature', () => feature);
SnapAdmin.Feature = SnapAdmin.Service('feature');
SnapAdmin.Service().register('repositoryFactory', () => repositoryFactory);

// Provide all services
SnapAdmin.Service().list().forEach(serviceKey => {
    config.global.provide[serviceKey] = SnapAdmin.Service(serviceKey);
});

// Set important functions for SnapAdmin Core
SnapAdmin.Application.view = {
...
SnapAdmin.State.commit('context/setApiApiPath', '/api');
SnapAdmin.State.commit('context/setApiApiResourcePath', '/api/v3');
SnapAdmin.State.commit('context/setApiAssetsPath', '');
SnapAdmin.State.commit('context/setApiLanguageId', '2fbb5fe2e29a4d70aa5854ce7ce3e20b');
SnapAdmin.State.commit('context/setApiInheritance', false);
SnapAdmin.State.commit('context/setApiSystemLanguageId', '2fbb5fe2e29a4d70aa5854ce7ce3e20b');
SnapAdmin.State.commit('context/setApiLiveVersionId', '0fa91ce3e96a4bc2be4bd9ce752c3425');
SnapAdmin.Context.api.authToken = {
