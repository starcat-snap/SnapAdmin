...
use SnapAdmin\Core\Installer\Requirements\Struct\RequirementCheck;
use SnapAdmin\Core\Installer\Requirements\Struct\RequirementsCheckCollection;
use SnapAdmin\Core\Installer\Requirements\Struct\SystemCheck;
...
class EnvironmentRequirementsValidator implements RequirementsValidatorInterface
{
    ...
    {
        $platform = $this->composer->getRepositoryManager()->getLocalRepository()->findPackage('snapadmin/platform', '*');
        if (!$platform) {
            $platform = $this->composer->getRepositoryManager()->getLocalRepository()->findPackage('snapadmin/core', '*');
        }
        if (!$platform) {
