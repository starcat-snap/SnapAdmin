...
class SetupDatabaseAdapter
{
    ...
    public function initializeSnapAdminDb(Connection $connection, ?string $database = null): bool
    {
        if (!$hasSnapAdminTables = $this->hasSnapAdminTables($connection, $database)) {
            $connection->executeStatement($this->getBaseSchema());
        }

        return !$hasSnapAdminTables;
    }

    public function hasSnapAdminTables(Connection $connection, ?string $database = null): bool
    {
        if ($database) {
