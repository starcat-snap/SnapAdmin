...
## [services.cache (`SnapAdmin\Core\Framework\Adapter\Cache\Script\Facade\CacheInvalidatorFacade`)](https://github.com/snap/platform/blob/trunk/src/Core/Framework/Adapter/Cache/Script/Facade/CacheInvalidatorFacade.php) {#cacheinvalidatorfacade}
...
## [services.writer (`SnapAdmin\Core\Framework\DataAbstractionLayer\Facade\RepositoryWriterFacade`)](https://github.com/snap/platform/blob/trunk/src/Core/Framework/DataAbstractionLayer/Facade/RepositoryWriterFacade.php) {#repositorywriterfacade}

The `writer` service allows you to write data, that is stored inside snap.
...
* The `upsert()` method allows you to create or update entities inside the database.
  ...
  If you pass an `id` in the payload it will do an update if an entity with that `id` already exists, otherwise it will
  be a create.
* **Returns
  ** [`SnapAdmin\Core\Framework\DataAbstractionLayer\Event\EntityWrittenContainerEvent`](https://github.com/snap/platform/blob/trunk/src/Core/Framework/DataAbstractionLayer/Event/EntityWrittenContainerEvent.php)
...
* **Arguments:**
    ...
    * *`array`* **payload**: The payload you want to upsert, as a list of associative arrays, where each associative
      array represents the payload for one entity.
* **Examples:**
    * Create a new entity.
...
* **Arguments:**
    ...
    * *`array`* **payload**: The primary keys of the entities you want to delete, as a list of associative arrays,
      associative array represents the primary keys for one entity.
* **Examples:**
    * Delete an entity.
...
* **Returns
  ** [`SnapAdmin\Core\Framework\Api\Sync\SyncResult`](https://github.com/snap/platform/blob/trunk/src/Core/Framework/Api/Sync/SyncResult.php)
...
## [services.response (`SnapAdmin\Core\Framework\Script\Api\ScriptResponseFactoryFacade`)](https://github.com/snap/platform/blob/trunk/src/Core/Framework/Script/Api/ScriptResponseFactoryFacade.php) {#scriptresponsefactoryfacade}
...
* **Returns
  ** [`SnapAdmin\Core\Framework\Script\Api\ScriptResponse`](https://github.com/snap/platform/blob/trunk/src/Core/Framework/Script/Api/ScriptResponse.php)
...
* The `render()` method allows you to render a twig view with the parameters you provide and create a
  StorefrontResponse.

  Note that the `render()` method will throw an exception if it is called from outside a `SalesChannelContext` (e.g.
  from an `/api` route)
  or if the Storefront-bundle is not installed.
* **Returns
  ** [`SnapAdmin\Core\Framework\Script\Api\ScriptResponse`](https://github.com/snap/platform/blob/trunk/src/Core/Framework/Script/Api/ScriptResponse.php)

  The created response object with the rendered template as content, remember to assign it to the hook
  with `hook.setResponse()`.
* **Arguments:**
    * *`string`* **view**: The name of the twig template you want to render
      e.g. `@Storefront/storefront/page/content/detail.html.twig`
    * *`array`* **parameters**: The parameters you want to pass to the template, ensure that you pass the `page`
      parameter from the hook to the templates.
