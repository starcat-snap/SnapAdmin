...
use SnapAdmin\Core\Framework\Api\Controller\Exception\PermissionDeniedException;
use SnapAdmin\Core\Framework\FrameworkException;
...
class DomainExceptionRule implements Rule
{
    ...
    private function validateDomainExceptionClass(Node\Expr\StaticCall $node, Scope $scope): array
    {
        ...
        if (!\str_starts_with($exceptionClass, 'SnapAdmin\\Core\\')) {
            return [];
        }
        ...
        if (!$exception->isSubclassOf(HttpException::class)) {
            return [
                RuleErrorBuilder::message(\sprintf('Domain exception class %s has to extend the \SnapAdmin\Core\Framework\HttpException class', $exceptionClass))->build(),
            ];
        }

        $reflection = $scope->getClassReflection();
        \assert($reflection !== null);
        if (!\str_starts_with($reflection->getName(), 'SnapAdmin\\Core\\')) {
            return [];
        }
        ...
        $expected = \sprintf('SnapAdmin\\Core\\%s\\%s\\%sException', $domain, $sub, $sub);
