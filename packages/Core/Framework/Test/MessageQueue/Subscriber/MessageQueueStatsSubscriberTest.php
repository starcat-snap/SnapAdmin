...
use SnapAdmin\Core\Framework\Test\MessageQueue\fixtures\BarMessage;
use SnapAdmin\Core\Framework\Test\MessageQueue\fixtures\FooMessage;
use SnapAdmin\Core\Framework\Test\MessageQueue\fixtures\NoHandlerMessage;
...
class MessageQueueStatsSubscriberTest extends TestCase
{
    ...
    public function testListener(): void
    {
        ...
        $pool = $this->getContainer()
            ->get('snap.increment.gateway.registry')
            ->get(IncrementGatewayRegistry::MESSAGE_QUEUE_POOL);

        $pool->reset('message_queue_stats');

        /** @var MessageBusInterface $bus */
        $bus = $this->getContainer()->get('messenger.bus.test_snap');
