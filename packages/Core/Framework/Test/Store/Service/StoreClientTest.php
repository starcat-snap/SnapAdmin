...
class StoreClientTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->setLicenseDomain('snap-test');
    ...
    public function testSignPayloadWithAppSecret(): void
    {
        ...
        static::assertEquals([
            'snapVersion' => $this->getSnapAdminVersion(),
            'language' => 'en-GB',
            'domain' => 'snap-test',
    ...
    public function testItUpdatesUserTokenAfterLogin(): void
    {
        ...
        $this->storeClient->loginWithSnapAdminId('snapId', 'password', $this->storeContext);
        ...
        static::assertEquals([
            'snapId' => 'snapId',
            'password' => 'password',
            'snapUserId' => $contextSource->getUserId(),
        ], \json_decode($lastRequest->getBody()->getContents(), true, flags: \JSON_THROW_ON_ERROR));
    ...
    public function testItRequestsUpdatesForLoggedInUser(): void
    {
        ...
        static::assertEquals(
            $this->getStoreTokenFromContext($this->storeContext),
            $lastRequest->getHeader('X-SnapAdmin-Platform-Token')[0],
        );
    }
    ...
    public function testItRequestsUpdateForNotLoggedInUser(): void
    {
        ...
        static::assertFalse($lastRequest->hasHeader('X-SnapAdmin-Platform-Token'));
    }
    ...
    public function testItReturnsUserInfo(): void
    {
        $userInfo = [
            ...
            'avatarUrl' => 'https://avatar.snap.com/john-doe.png',
        ];
    ...
    public function testMissingConnectionBecauseYouAreInGermanCellularInternet(): void
    {
        $this->getRequestHandler()->append(new ConnectException(
            'cURL error 7: Failed to connect to api.snap.com port 443 after 4102 ms: Network is unreachable (see https://curl.haxx.se/libcurl/c/libcurl-errors.html) for https://api.snap.com/swplatform/pluginupdates?snapVersion=6.4.12.0&language=de-DE&domain=',
