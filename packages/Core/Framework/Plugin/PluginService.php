...
class PluginService
{
    ...
    public function refreshPlugins(Context $snapContext, IOInterface $composerIO): ExceptionCollection
    {
        ...
        $installedPlugins = $this->getPlugins(new Criteria(), $snapContext);
        ...
        foreach ($pluginsFromFileSystem as $pluginFromFileSystem) {
            ...
            $pluginData['translations'] = $this->getTranslations($snapContext, $extra);
        ...
        if ($plugins !== []) {
            foreach ($plugins as $plugin) {
                try {
                    $this->pluginRepo->upsert([$plugin], $snapContext);
                } catch (SnapAdminHttpException $exception) {
        ...
        if (\count($deletePluginIds) !== 0) {
            ...
            $this->pluginRepo->delete($deletePlugins, $snapContext);
        }
    ...
    private function getTranslations(Context $context, array $extra): array
    {
         ...
         * @example payload
         * {
         *     "snap-plugin-class":"Swag\\MyDemoData\\MyDemoData",
         *     "label":{
