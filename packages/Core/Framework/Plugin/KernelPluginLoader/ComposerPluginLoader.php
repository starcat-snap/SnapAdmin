...
namespace SnapAdmin\Core\Framework\Plugin\KernelPluginLoader;
...
class ComposerPluginLoader extends KernelPluginLoader
{
    protected function loadPluginInfos(): void
    {
        ...
        foreach ($composerPlugins as $composerName) {
            ...
            $pluginClass = $composerJson['extra']['snap-plugin-class'] ?? '';

            if (\defined('\STDERR') && ($pluginClass === '' || !\class_exists($pluginClass))) {
                \fwrite(\STDERR, \sprintf('Skipped package %s due invalid "snap-plugin-class" config', $composerName) . \PHP_EOL);
