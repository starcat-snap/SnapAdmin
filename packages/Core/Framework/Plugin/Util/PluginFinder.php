...
use SnapAdmin\Core\Framework\Plugin\Composer\PackageProvider;
use SnapAdmin\Core\Framework\Plugin\Exception\ExceptionCollection;
use SnapAdmin\Core\Framework\Plugin\Exception\PluginComposerJsonInvalidException;
use SnapAdmin\Core\Framework\Plugin\Struct\PluginFromFileSystemStruct;
...
class PluginFinder
{
    ...
    private function loadLocalPlugins(string $pluginDir, IOInterface $composerIO, ExceptionCollection $errors): array
    {
        ...
        try {
            ...
            foreach ($filesystemPlugins as $filesystemPlugin) {
                ...
                if (!$this->isSnapAdminPluginType($package) || !$this->isPluginComposerValid($package)) {
    ...
    private function isSnapAdminPluginType(CompletePackageInterface $package): bool
    {
    ...
    private function enrichWithVendorPlugins(
        ...
        foreach ($composerPackages as $composerPackage) {
            if (!$this->isSnapAdminPluginType($composerPackage)) {
                continue;
            }
        ...
        if ($this->isSnapAdminPluginType($root) && $this->isPluginComposerValid($root)) {
