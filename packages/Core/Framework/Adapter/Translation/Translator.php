...
class Translator extends AbstractTranslator
{
    ...
    public function getCatalogue(?string $locale = null): MessageCatalogueInterface
    {
        ...
        if ($this->isSnapAdminLocaleCatalogue($catalog) && !$this->isFallbackLocaleCatalogue($catalog, $localization)) {
            ...
            // or locale is symfony style locale so we shouldn't add snap fallbacks as it may lead to circular references
            $fallbackLocale = null;
        }
    ...
    public function trans(string $id, array $parameters = [], ?string $domain = null, ?string $locale = null): string
    {
        ...
        if ($this->fineGrainedCache) {
            ...
            foreach (array_keys($this->keys) as $trace) {
                $this->traces[$trace]['snap.translator'] = true;
            }
        }
    ...
    /**
     * SnapAdmin uses dashes in all locales
     * if the catalogue does not contain any dashes it means it is a symfony fallback catalogue
     * in that case we should not add the snap fallback catalogue as it would result in circular references
     */
    private function isSnapAdminLocaleCatalogue(MessageCatalogueInterface $catalog): bool
    {
