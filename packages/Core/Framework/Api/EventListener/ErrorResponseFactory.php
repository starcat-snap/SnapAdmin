...
namespace SnapAdmin\Core\Framework\Api\EventListener;
 ...
 *
 * @phpstan-import-type ErrorData from SnapAdminHttpException as SnapAdminExceptionData
 */
#[Package('core')]
class ErrorResponseFactory
{
    ...
    /**
     * @return array<DefaultExceptionData|SnapAdminExceptionData>
     */
    public function getErrorsFromException(\Throwable $exception, bool $debug = false): array
    {
        if ($exception instanceof SnapAdminHttpException) {
            $errors = [];
    ...
    private function getStatusCodeFromException(\Throwable $exception): int
    {
        ...
        if ($exception instanceof SnapAdminHttpException || $exception instanceof HttpException) {
